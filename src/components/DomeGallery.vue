<template>
  ...
  >
    <main ref="mainRef" class="dome-main-content">
      <div class="dome-perspective-container">
        <div
          ref="sphereRef"
          class="dome-sphere"
      ...
      <div ref="viewerRef" class="dome-viewer">
        <div
          ref="scrimRef"
          class="dome-scrim"
          :style="{ backdropFilter: 'blur(3px)' }"
        />
        <div ref="frameRef" class="dome-frame" :style="{ borderRadius: 'var(--enlarge-radius, 32px)' }" />
      </div>
    </main>
  </div>
</template>
...
const imagesSource = computed(() => props.images || []);
...
const openItemFromElement = (el: HTMLElement) => {
  ...
  refDiv.className = 'dome-item-reference';
  ...
  requestAnimationFrame(() => {
    ...
    scrimRef.value?.style.setProperty('opacity', '1');
    scrimRef.value?.style.setProperty('pointer-events', 'auto');
  });
...
const closeEnlargedImage = () => {
  ...
  if (!el) return;
  const parent = el.parentElement;
  const overlay = viewerRef.value?.querySelector('.dome-enlarge') as HTMLElement;
  if (!overlay || !parent) return;
  const refDiv = parent.querySelector('.dome-item-reference');
  ...
  if (!originalPos) {
    ...
    scrimRef.value?.style.setProperty('opacity', '0');
    scrimRef.value?.style.setProperty('pointer-events', 'none');
...
onMounted(() => {
  ...
  resizeObserver = new ResizeObserver(entries => {
    ...
    const overlay = viewerRef.value?.querySelector('.dome-enlarge') as HTMLElement | null;
